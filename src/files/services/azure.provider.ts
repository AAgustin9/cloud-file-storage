...
export class AzureProvider implements StorageInterface {
  ...
  constructor() {
    ...
    if (this.isConfigured) {
      ...
      this.logger.warn(
        'Azure Blob Storage is not properly configured. Check your environment variables.',
      );
    }
  }
  ...
  async upload(file: Express.Multer.File, key: string): Promise<string> {
    try {
      if (!this.isConfigured) {
        throw new Error(
          'Azure Blob Storage is not configured. Please check your environment variables.',
        );
      }
